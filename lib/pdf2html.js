...
Pdf2Html.prototype._copyFonts = function (startPath) {
  ...
  console.log(startPath + ' - start copy fonts');
...
Pdf2Html.prototype._getFontsFromPdf = function (file, fontOutput) {
  var dirname = path.dirname(file);
  var extractCommand = util.format("pdf2htmlEX '%s'--font-format ttf --embed-font 0", file);
  ...
  result.stdout.split("\n").forEach(function (line) {
    if (line.match(/^extracting font (.*)$/)) {
      var font = RegExp.$1;
      var fontPath = options.basedir + '/' + font;
      console.log(fontPath);
      fs.renameSync(fontPath, dirname + '/fonts/' + font);
    }
  });

  this._removeImages(options.basedir, '.jpg');
  this._removeImages(options.basedir, '.pam');
  this._removeImages(options.basedir, '.png');
...
Pdf2Html.prototype._getFontName = function (font) {
  var getFontName = util.format("fontforge -script "+__dirname+"/../getFontName.pe '%s' 2> /dev/null", font);
...
Pdf2Html.prototype._getStyledFonts = function (startPath) {
  ...
  _.forEach(data, function (value) {
    ...
    text += "@font-face {font-family: \""+value.fontname+"\"; src: url(\"fonts/"+encodeURIComponent(value.basename)+".ttf\");}\n"
  })
